...
public class GenerateLevel : MonoBehaviour {
    ...
    //bool IsOccupied(int y_upper_left, int x_upper_left, int y_lower_right, int x_lower_right) {
    bool IsOccupied(int room_y, int room_x, int room_index) {
        int room_height = RoomDistance(room_index, 0);
        int room_width = RoomDistance(room_index, 1);
    ...
    int RoomDistance(int room_index, int axis) {
        return roomSpace[room_index, axis] / 2 + 1;
    }

    int CheckIfEdge(int new_room_y, int new_room_x, int new_room_index, GameObject inst) {
        int edge = -1;
        ...
        dist = RoomDistance(new_room_index, 1);
    ...
    void AddRoom(GameObject currentRoom, int direction, int room_x, int room_y, int room_index, int current_depth) {
        ...
        int new_room_index = random.Next(5, numberOfRooms);
        switch(direction) {
            case 0:
        ...
        ++current_rooms;
        CheckIfEdge(new_room_y, new_room_x, new_room_index, inst_r);
    ...
    void CreateLevel() {
        ...
        GameObject inst = AddToGrid(roomList[0], 0, 0);
        occupiedSpace[o_y, o_x] = true;

        int[] directions = { 0, 1, 2, 3 };
        directions = directions.OrderBy(x => random.Next()).ToArray();
        //int paths = random.Next(1, 3) + 1;
        int paths = 4;
        for(int i = 0; i < 4; ++i) {
            edges[i, 0] = o_y;
            edges[i, 1] = o_x;
        ...
        for(int i = 0; i < paths; ++i) {
            AddRoom(inst, (directions[i] + 2) % 4, o_x, o_y, 0, 0);
        }
    }
    ...
    void FinishLevel() {
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            if(i % 2 == 1) {
                ...
                path_x -= sign * 10;
                addition = RoomDistance(order[i], 1) * 10;
                GameObject inst = AddToGrid(roomList[order[i]], path_x + sign * addition, path_y);
                ...
                path_y -= sign * 10;
                addition = RoomDistance(order[i], 0) * 10;
                GameObject inst = AddToGrid(roomList[order[i]], path_x, path_y + sign * addition);
                RemoveWall(inst, i);
            }
        }
    }
    ...
    void Start() {
        // TODO: coridoare imbunatatite
        // TODO: verificare numar de camere minim (linked list?)
